...
class LabelTest(TestCase):
    ...
    def test_label_create_post_not_logged_in(self):
        self.client.post(reverse('user_logout'))
        response = self.client.post(reverse('label_create'), data=self.label, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, _('Password'))
    ...
    def test_label_create_post_incorrect(self):
        ...
        response = self.client.post(reverse('label_create'), data={'name': 'a'*500}, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, _('Create'))
    ...
    def test_label_update_get_not_logged_in(self):
        self.client.post(reverse('user_logout'))
        response = self.client.get(reverse('label_update', args=[self.id]), follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, _('Password'))

    def test_label_update_post_not_logged_in(self):
        self.client.post(reverse('user_logout'))
        response = self.client.post(reverse('label_update', args=[self.id]), data=self.label, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, _('Password'))
    ...
    def test_label_update_post_correct_code(self):
        response = self.client.post(reverse('label_update', args=[self.id]), data=self.label)
        self.assertEqual(response.status_code, 302)
    ...
    def test_label_update_post_incorrect(self):
        ...
        response = self.client.post(reverse('label_update', args=[self.id]), data={'name': 'a'*500}, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, _('update'))
    ...
    def test_label_delete_post_not_logged_in(self):
        self.client.post(reverse('user_logout'))
        response = self.client.get(reverse('label_delete', args=[self.id]), follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, _('Password'))
    ...
    def test_label_delete_post_correct_content(self):
        response = self.client.post(reverse('label_delete', args=[self.id]), follow=True)
        self.assertEqual(response.status_code, 200)
