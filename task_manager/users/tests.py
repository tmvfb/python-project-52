...
class UserTest(TestCase):
    def setUp(self):
        ...
        self.user_incorrect_pw = UserFactory.create_fake_user(password="pw")
        self.user_incorrect_un = UserFactory.create_fake_user(username="")
        self.login_data = {
    ...
    def test_user_create_post_correct_content(self):
        response = self.client.post(
            reverse("user_create"), data=self.user, follow=True
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_user_login_code(self):
        response = self.client.post(
            reverse("user_login"), data=self.login_data
        )
        self.assertEqual(response.status_code, 302)

    def test_user_login_content(self):
        response = self.client.post(
            reverse("user_login"), data=self.login_data, follow=True
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_user_update_post_not_logged_in(self):
        response = self.client.get(
            reverse("user_update", args=[self.id]), follow=True
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, _("You must login to be able to update your profile")
        )
    ...
    def test_user_update_get_wrong_user(self):
        ...
        response = self.client.get(
            reverse("user_update", args=[self.wrong_id])
        )
        self.assertEqual(response.status_code, 302)
        ...
        response = self.client.get(
            reverse("user_update", args=[self.wrong_id]), follow=True
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response,
            _(
                "Sorry, you don&#x27;t have permissions to update other users&#x27; data"  # noqa: E501
            ),
        )
    ...
    def test_user_update_post_correct_code(self):
        ...
        response = self.client.post(
            reverse("user_update", args=[self.id]), data=self.user
        )
        self.assertEqual(response.status_code, 302)
    ...
    def test_user_update_post_incorrect(self):
        ...
        response = self.client.post(
            reverse("user_update", args=[self.id]), data=self.user_incorrect_un
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
    ...
    def test_user_delete_post_not_logged_in(self):
        response = self.client.get(
            reverse("user_delete", args=[self.id]), follow=True
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, _("You must login to be able to delete your profile")
        )
    ...
    def test_user_delete_get_wrong_user(self):
        ...
        response = self.client.get(
            reverse("user_delete", args=[self.wrong_id])
        )
        self.assertEqual(response.status_code, 302)
        ...
        response = self.client.get(
            reverse("user_delete", args=[self.wrong_id]), follow=True
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response,
